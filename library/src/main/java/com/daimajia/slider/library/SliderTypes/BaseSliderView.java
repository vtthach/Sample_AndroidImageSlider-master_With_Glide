...
public abstract class BaseSliderView {
    ...
    private RequestBuilder<Drawable> rq;
    ...
    protected BaseSliderView(Context context) {
        mContext = context;
        requestManager = Glide.with(mContext.getApplicationContext());
    }
    ...
    protected void bindEventAndShow(final View v, ImageView targetImageView) {
        ...
        if (mUrl != null) {
            rq = requestManager.load(mUrl);
        } else if (mFile != null) {
            rq = requestManager.load(mFile);
        } else if (mRes != 0) {
            rq = requestManager.load(mRes);
        } else {
            return;
        }
        ...
        rq = rq.listener(new RequestListener<Drawable>() {
            @Override
     ...
     */
    public RequestBuilder<Drawable> getPicasso() {
        return rq;
    }
}
